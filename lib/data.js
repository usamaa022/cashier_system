...
// Optimized getStoreItems with caching and reduced queries
...
export async function getStoreItems(forceRefresh = false) {
  try {
    ...
    console.log("🔍 getStoreItems: Fetching fresh data...");
    ...
    for (const doc of snapshot.docs) {
      try {
        ...
        // Get bill number
        let boughtBillNumber = data.boughtBillNumber || data.billNumber || data.bill_number || 'N/A';
...
export async function createBoughtBill(
  ...
  try {
    ...
    const isUpdating = existingBillNumber !== null;
    const billNumber = existingBillNumber || await generateBillNumber();
    ...
    // Only add createdAt for new bills
    if (!isUpdating) {
    ...
    if (isUpdating) {
      ...
      // First, find the bill by billNumber
      ...
      if (billsSnapshot.empty) {
        throw new Error(`Bill #${billNumber} not found for updating`);
      }
      ...
      // Handle store items updates
      // First, get all store items for this bill
      ...
      // For existing store items, we need to handle quantity changes
      // This is complex - you might want to:
      // 1. Delete all existing store items for this bill and recreate them, OR
      // 2. Update quantities appropriately
      
      // Option 1: Delete and recreate (simpler but loses history)
      // Uncomment if you want to use this approach:
      /*
      // Delete all existing store items for this bill
      for (const storeItemDoc of storeItemsSnapshot.docs) {
        await deleteDoc(doc(db, STORE_ITEMS_COLLECTION, storeItemDoc.id));
      }
      ...
      */
      ...
      // Option 2: Update quantities (more complex but preserves history)
      // This is a simplified version - you may need to adjust based on your business logic
      for (const item of bill.items) {
        // Check if this item already exists in store with same barcode and expire date
...
export async function updateBoughtBill(billNumber, updates) {
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      // Handle items if they exist in updates
      if (updates.items && Array.isArray(updates.items)) {
        updates.items = updates.items.map(item => ({
          ...
          basePrice: Math.round((item.basePriceUSD || 0) * (updates.exchangeRate || 1500)),
          netPrice: Math.round((item.netPriceUSD || 0) * (updates.exchangeRate || 1500)),
          outPrice: Math.round((item.outPriceUSD || 0) * (updates.exchangeRate || 1500)),
        }));
      }
      ...
      await syncBoughtReturnsWithBill(billNumber, updates.items);
...
export async function syncBoughtReturnsWithBill(billNumber, updatedItems) {
  try {
    // Get all bought returns for this bill
    const returnsRef = collection(db, "boughtReturns");
    const q = query(returnsRef, where("billNumber", "==", billNumber));
    const snapshot = await getDocs(q);

    // For each return, check if the item's quantity was updated
    for (const returnDoc of snapshot.docs) {
      const returnData = returnDoc.data();
      const returnItem = updatedItems.find(item => item.barcode === returnData.barcode);
      ...
      if (returnItem) {
        ...
        if (originalItem) {
          ...
          // If the available quantity is less than the returned quantity, adjust the return
