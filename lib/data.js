...
export async function createSoldBill(billData, existingBillNumber = null) {
  try {
    ...
    // Generate bill number with new format (YYNNN)
    let billNumber;
    if (existingBillNumber) {
      ...
      let nextSequence = 100; // Start from 100 for each year

      if (!snapshot.empty) {
        const lastBill = snapshot.docs[0].data();
        const lastBillNumber = lastBill.billNumber;

        if (typeof lastBillNumber === "number") {
          const lastYear = Math.floor(lastBillNumber / 1000);
          const lastSequence = lastBillNumber % 1000;

          if (lastYear === yearPrefix) {
            nextSequence = lastSequence + 1;
          }
        } else if (typeof lastBillNumber === "string" && lastBillNumber.length >= 5) {
          const lastYearStr = lastBillNumber.slice(0, 2);
          const lastSequenceStr = lastBillNumber.slice(2);
      ...
      billNumber = yearPrefix * 1000 + nextSequence;
    }
...
export async function generateUniqueReturnBillNumber() {
  try {
    // Get all existing return bill numbers
    const returnsRef = collection(db, RETURNS_COLLECTION);
    const snapshot = await getDocs(returnsRef);
    ...
    do {
      ...
      newNumber = `RET-${timestamp}-${counter.toString().padStart(3, '0')}`;
      counter++;
    } while (existingNumbers.has(newNumber) && counter < 1000);

    return newNumber;
  } catch (error) {
    console.error("Error generating unique return bill number:", error);
    // Fallback to timestamp-based number
    return `RET-${Date.now()}`;
  }
}

export async function returnItemsToStore(pharmacyId, items, note = "") {
  try {
    if (!pharmacyId) {
    ...
    // Store the note with the return bill
    const returnBillNote = note || "";

    // Create a return document for each item
    const returnPromises = items.map(async (item) => {
      if (!item.barcode || !item.name || !item.returnQuantity || !item.returnPrice) {
        throw new Error("Invalid item data: barcode, name, returnQuantity, and returnPrice are required");
      }
      ...
      // Calculate remaining quantity in sold bill
      let remainingQuantity = (item.quantity || 0) - (item.returnQuantity || 0);
      ...
      const returnRecord = {
        ...
        outPrice: item.outPrice || 0,
        expireDate: expireDateTimestamp,
        pharmacyReturnBillNumber: item.pharmacyReturnBillNumber || "",
        returnDate: serverTimestamp(),
        paymentStatus: "Unpaid",
        isConsignment: item.isConsignment || false,
        consignmentOwnerId: item.consignmentOwnerId || null,
        remainingQuantityInBill: remainingQuantity,
      };
    ...
    // Wait for all returns to be processed
    const results = await Promise.all(returnPromises);
    return { returnBillNumber, results };
  } catch (error) {
...
// lib/data.js - Update the receiveTransport function
// lib/data.js - Update the receiveTransport function to store adjusted quantities
...
// lib/data.js - Update the getTransports function
