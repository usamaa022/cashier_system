...
// Add these updated functions to your data.js

// Updated createBoughtBill function for proper dual currency handling
  ...
  try {
    ...
    const itemsWithExpireDate = billItems.map((item) => {
      ...
      if (item.expireDate) {
        if (typeof item.expireDate === "string") {
          ...
          expireDateTimestamp = new Timestamp(
            ...
            item.expireDate.nanoseconds
          );
        }
      }

      // Calculate base prices based on currency
      let basePriceUSD = 0;
      let basePriceIQD = 0;
      let netPriceUSD = 0;
      let netPriceIQD = 0;
      ...
      if (currency === "USD") {
        ...
        basePriceIQD = priceValue * exchangeRate;
        netPriceUSD = item.netPriceUSD || basePriceUSD;
        netPriceIQD = netPriceUSD * exchangeRate;
        outPriceUSD = item.outPriceUSD || basePriceUSD * 1.5;
        outPriceIQD = outPriceUSD * exchangeRate;
      } else {
        basePriceIQD = priceValue;
        basePriceUSD = priceValue / exchangeRate;
        netPriceIQD = item.netPriceIQD || basePriceIQD;
        netPriceUSD = netPriceIQD / exchangeRate;
        outPriceIQD = item.outPriceIQD || basePriceIQD * 1.5;
        outPriceUSD = outPriceIQD / exchangeRate;
      }
      ...
      return {
        ...
        basePriceIQD: Math.round(basePriceIQD),
        netPriceUSD: Math.round(netPriceUSD * 100) / 100,
        netPriceIQD: Math.round(netPriceIQD),
        outPriceUSD: Math.round(outPriceUSD * 100) / 100,
        outPriceIQD: Math.round(outPriceIQD),
    ...
    const totalTransportFeeUSD =
      currency === "USD"
        ? additionalData.transportFee || 0
        : (additionalData.transportFee || 0) / exchangeRate;
    const totalTransportFeeIQD =
      currency === "IQD"
        ? additionalData.transportFee || 0
        : (additionalData.transportFee || 0) * exchangeRate;
    const totalExternalExpenseUSD =
      currency === "USD"
        ? additionalData.externalExpense || 0
        : (additionalData.externalExpense || 0) / exchangeRate;
    const totalExternalExpenseIQD =
      currency === "IQD"
        ? additionalData.externalExpense || 0
        : (additionalData.externalExpense || 0) * exchangeRate;
      ...
      for (const item of bill.items) {
        const existing = await getDocs(
          query(
            ...
            where("barcode", "==", item.barcode),
            where("expireDate", "==", item.expireDate),
            where("netPriceUSD", "==", item.netPriceUSD),
            where("netPriceIQD", "==", item.netPriceIQD),
...
// lib/data.js - Updated createSoldBill function with dual currency support
export async function createSoldBill(billData) {
  try {
    const {
      ...
      exchangeRate = 1500, // Add exchange rate parameter
    } = billData;
    ...
    const finalCreatedByName =
      createdByName && createdByName !== "Unknown User" ? createdByName : "System User";

    // Process items - keep original currency information
    const processedItems = preparedItems.map((item) => {
      const expireDateTimestamp = toFirestoreTimestamp(item.expireDate);
      
      // Determine the selling price based on the item's original currency
      let sellingPrice = 0;
      let sellingPriceUSD = 0;
      let sellingPriceIQD = 0;
      
      if (item.originalCurrency === "USD" || item.currency === "USD") {
        sellingPriceUSD = parseFloat(item.price) || parseFloat(item.outPriceUSD) || 0;
        sellingPriceIQD = sellingPriceUSD * exchangeRate;
        sellingPrice = sellingPriceUSD;
      } else {
        sellingPriceIQD = parseFloat(item.price) || parseFloat(item.outPriceIQD) || 0;
        sellingPriceUSD = sellingPriceIQD / exchangeRate;
        sellingPrice = sellingPriceIQD;
      }
      ...
      return {
        ...
        quantity: parseInt(item.quantity) || 1,
        netPrice: item.netPrice || 0,
        netPriceUSD: item.netPriceUSD || (item.netPrice / exchangeRate),
        netPriceIQD: item.netPriceIQD || (item.netPrice * exchangeRate),
        outPrice: item.outPrice || sellingPrice,
        outPriceUSD: item.outPriceUSD || sellingPriceUSD,
        outPriceIQD: item.outPriceIQD || sellingPriceIQD,
        price: sellingPrice,
        expireDate: expireDateTimestamp,
        batchId: item.batchId || null,
        isConsignment: isConsignment,
        consignmentOwnerId: isConsignment ? pharmacyId : null,
        originalCurrency: item.originalCurrency || item.currency || currency,
    ...
    // Calculate total amount in both currencies
    const totalAmountUSD = processedItems.reduce((sum, item) => 
      sum + (item.outPriceUSD * item.quantity), 0);
    const totalAmountIQD = processedItems.reduce((sum, item) => 
      sum + (item.outPriceIQD * item.quantity), 0);
    ...
    // Deduct quantities from store based on original currency
    for (const item of processedItems) {
      const storeItemsRef = collection(db, STORE_ITEMS_COLLECTION);
      
      // Find matching store items by barcode and original currency
      let q;
      ...
      if (snapshot.empty) {
        console.warn(`No matching store items found for ${item.name} (${item.barcode}) in ${item.originalCurrency}`);
        // Try fallback search without currency filter
        ...
        if (fallbackSnapshot.empty) {
          throw new Error(`Insufficient stock for ${item.name} (${item.barcode}). No items found in store.`);
        }
        ...
        for (const storeItem of matchingItems) {
          ...
          console.log(`Deducting ${deductQty} from ${item.name} (${storeItem.id}), remaining: ${remainingQty - deductQty}`);
...
// lib/data.js - Updated updateBoughtBill function
export async function updateBoughtBill(billNumber, updates) {
  try {
    ...
    // Get currency and exchange rate from updates or existing bill
    ...
    if (updates.items && Array.isArray(updates.items)) {
      processedItems = updates.items.map((item) => {
        ...
        let basePriceUSD = item.basePriceUSD || 0;
        let basePriceIQD = item.basePriceIQD || 0;
        let netPriceUSD = item.netPriceUSD || 0;
        let netPriceIQD = item.netPriceIQD || 0;
        let outPriceUSD = item.outPriceUSD || 0;
        let outPriceIQD = item.outPriceIQD || 0;

        if (currency === "USD") {
          basePriceUSD = parseFloat(item.price) || item.basePriceUSD || 0;
          basePriceIQD = basePriceUSD * exchangeRate;
          netPriceUSD = item.netPrice || basePriceUSD;
          netPriceIQD = netPriceUSD * exchangeRate;
          outPriceUSD = item.outPriceUSD || basePriceUSD * 1.5;
          outPriceIQD = outPriceUSD * exchangeRate;
        } else if (currency === "IQD") {
          basePriceIQD = parseFloat(item.price) || item.basePriceIQD || 0;
          basePriceUSD = basePriceIQD / exchangeRate;
    ...
    // Update store items if items changed
    if (updates.items && Array.isArray(updates.items)) {
      // First, remove old quantities from store
      const oldStoreItemsQuery = query(
        collection(db, STORE_ITEMS_COLLECTION),
        where("boughtBillNumber", "==", billNumber)
      );
      ...
      for (const oldItemDoc of oldStoreItemsSnapshot.docs) {
        ...
        const newItem = processedItems.find(item => 
          item.barcode === oldItem.barcode && 
          item.expireDate?.toDate?.()?.getTime() === oldItem.expireDate?.toDate?.()?.getTime()
        );
        
        if (newItem) {
          // Update existing store item
          await updateDoc(doc(db, STORE_ITEMS_COLLECTION, oldItemDoc.id), {
          ...
          // Remove item that's no longer in the bill
...
// Updated returnItemsToStore function
  ...
  try {
    ...
    const validatedItems = items.map((item, index) => {
      ...
      if (item.expireDate && item.expireDate !== 'N/A' && item.expireDate !== '') {
        try {
          if (typeof item.expireDate === 'string') {
            ...
            expireDateTimestamp = new Timestamp(item.expireDate.seconds, item.expireDate.nanoseconds || 0);
          }
        } catch (e) {
      ...
      return {
        ...
        returnPriceUSD: item.currency === "USD" ? returnPrice : 0,
        returnPriceIQD: item.currency === "IQD" ? returnPrice : 0,
      };
    });
...
// Updated updateBoughtReturnItems function
export async function updateBoughtReturnItems(returnId, items) {
  try {
    ...
    if (originalItem.currency === "USD" && updatedItem.returnPriceIQD !== 0) {
      throw new Error("For USD currency, IQD values must be 0.");
    }
    if (originalItem.currency === "IQD" && updatedItem.returnPriceUSD !== 0) {
      throw new Error("For IQD currency, USD values must be 0.");
    }

    // Restore the original quantity back to store
    if (originalItem.barcode && originalItem.returnQuantity) {
      const storeItemsRef = collection(db, STORE_ITEMS_COLLECTION);
    ...
    // Deduct the new quantity from store
...
// Updated deleteBoughtReturn function
export async function deleteBoughtReturn(returnId, barcode) {
  try {
    ...
    // If only one item, delete the entire return bill
    if (items.length === 1) {
      await deleteDoc(returnRef);
      return { success: true, message: "Return bill deleted." };
    }

    // Otherwise, remove only the selected item
