  ...
  try {
    ...
    for (const newItem of processedItems) {
      ...
      await updateDoc(doc(db, STORE_ITEMS_COLLECTION, existing.id), {
        ...
        // createdAt is intentionally NOT touched — preserves original purchase time
        updatedAt: serverTimestamp(),
      });
    }
...
export async function createBoughtReturn(companyId, items, note, returnBillNumber) {
  ...
  const finalReturnBillNumber = returnBillNumber || (await generateBoughtReturnBillNumber());
    ...
    for (const item of items) {
      ...
      // ── STEP 1: Ground-truth cap = originally purchased qty on this bill ──
      let originalBillQuantity = Number(item.quantity) || Number(item.originalQuantity) || 0;
      try {
        const billData = await getBoughtBillByNumber(billNumber);
        if (billData?.items?.length) {
          const billItem =
            billData.items.find((bi) => String(bi.barcode) === String(barcode)) ||
            billData.items.find((bi) => bi.name?.toLowerCase() === itemName.toLowerCase());
          if (billItem) originalBillQuantity = Number(billItem.quantity) || originalBillQuantity;
        }
      } catch (err) {
        console.warn("Could not fetch original bill, using item quantity as fallback:", err);
      }

      // ── STEP 2: How much has already been returned against this bill+barcode ──
      const priorReturnsSnap = await getDocs(
        query(
          collection(db, BOUGHT_RETURNS_COLLECTION),
          where("barcode", "==", barcode),
      ...
      priorReturnsSnap.docs.forEach((d) => {
        totalReturnedAlready += Number(d.data().returnQuantity) || 0;
      });

      const availableToReturn = originalBillQuantity - totalReturnedAlready;
      if (returnQuantity > availableToReturn) {
        throw new Error(
          `Cannot return ${returnQuantity} of ${itemName}. Original bill: ${originalBillQuantity}, ` +
          `already returned: ${totalReturnedAlready}, available to return: ${availableToReturn}`
        );
      }

      // ── STEP 3: Verify PHYSICAL stock exists before deducting (never over-deduct) ──
      const storeItemsRef = collection(db, STORE_ITEMS_COLLECTION);
      const storeQ = query(
        storeItemsRef,
        where("barcode", "==", barcode),
      ...
      const storeDocs = storeSnapshot.docs
        .map((d) => ({ id: d.id, ...d.data(), quantity: Number(d.data().quantity) || 0 }))
        .filter((d) => d.quantity > 0);

      const totalInStore = storeDocs.reduce((sum, d) => sum + d.quantity, 0);
      if (totalInStore < returnQuantity) {
        throw new Error(
          `Cannot return ${returnQuantity} of ${itemName}. Only ${totalInStore} units currently in store ` +
          `(some of the purchased quantity may already be sold).`
        );
      }

      // ── STEP 4: Deduct FIFO by expiry (delta only — never touches unrelated stock) ──
      const sorted = storeDocs.sort((a, b) => {
      ...
      for (const storeItem of sorted) {
        if (remaining <= 0) break;
        const deductQty = Math.min(remaining, storeItem.quantity);
        const newQty = storeItem.quantity - deductQty;
        if (newQty <= 0) {
    ...
    const totalReturnAmount = items.reduce(
      (sum, item) => sum + (Number(item.returnPrice) || 0) * (Number(item.returnQuantity) || 0),
      0
    );
    const totalReturnQty = items.reduce((sum, item) => sum + (Number(item.returnQuantity) || 0), 0);
    ...
    const returnData = {
      ...
      items: items.map((item) => ({
        ...
        returnPriceUSD: Number(item.returnPriceUSD) || 0,
        returnPriceIQD: Number(item.returnPriceIQD) || 0,
        billNumber: String(item.billNumber || ""),
        quantity: Number(item.quantity) || 0,
        netPrice: Number(item.netPrice) || 0,
        outPrice: Number(item.outPrice) || 0,
        expireDate: item.expireDate || null,
        isConsignment: Boolean(item.isConsignment) || false,
        consignmentOwnerId: item.consignmentOwnerId || null,
        currency: String(item.currency || "USD"),
        returnNote: String(note || ""),
        branch: String(item.branch || "Slemany"),
      ...
      totalReturnAmount,
      totalReturnQty,
      billNumber: String(items[0]?.billNumber || ""),
      createdBy: "system",
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
    };

    const docRef = await addDoc(collection(db, BOUGHT_RETURNS_COLLECTION), returnData);

    storeItemsCache = null;
    lastFetchTime = 0;

    return { id: docRef.id, returnBillNumber: finalReturnBillNumber, ...returnData };
  } catch (error) {
...
// ===================== NEW: deleteBoughtReturnItem =====================
// Item-aware delete: works with the real items[] array schema.
export async function deleteBoughtReturnItem(returnDocId, barcode) {
  try {
    ...
    if (restoreQty > 0) {
      ...
      if (!storeSnapshot.empty) {
        ...
        await addDoc(collection(db, STORE_ITEMS_COLLECTION), {
          ...
          originalCurrency: currency,
          branch,
          boughtBillNumber: item.billNumber || returnDocData.billNumber || null,
          createdAt: serverTimestamp(),
          updatedAt: serverTimestamp(),
        });
      }
    ...
    // ── Remove this item from the array; delete whole doc if it was the only one ──
