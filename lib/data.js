...
export async function getSaleBillById(billId) {
  try {
    ...
    console.log(`🔍 Getting sale bill by ID: ${billId}`);
    const billDocRef = doc(db, SOLD_BILLS_COLLECTION, billId);
    const billSnap = await getDoc(billDocRef);
    let billData;
    if (billSnap.exists()) {
      console.log(`✅ Found sale bill by document ID: ${billId}`);
      billData = billSnap.data();
    } else {
      console.log(`🔍 Document not found by ID, trying to find by billNumber: ${billId}`);
      const q = query(collection(db, SOLD_BILLS_COLLECTION), where("billNumber", "==", billId), limit(1));
      const snapshot = await getDocs(q);
      if (!snapshot.empty) {
        console.log(`✅ Found sale bill by billNumber: ${billId}`);
    ...
    const processedItems = (billData.items || []).map(item => {
      ...
      if (item.expireDate) {
        if (item.expireDate.toDate) {
          ...
          expireDate = new Timestamp(item.expireDate.seconds, item.expireDate.nanoseconds);
        }
      }
      return {
        ...item,
...
export async function updateSaleBillQuantities(billId, barcode, remainingQty, totalReturnedQty) {
  try {
    ...
    console.log(`🔄 Updating sale bill quantities:`, {
      billId,
      barcode,
      remainingQty,
      totalReturnedQty
    ...
    console.log(`✅ Sale bill quantities updated successfully`);
    return {
      success: true,
      billId,
      barcode,
