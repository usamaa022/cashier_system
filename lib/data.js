...
// Format date function - updated to handle various formats and prevent errors
const formatDate = (date) => {
  ...
  try {
    ...
    // Handle Firestore Timestamp (most common in your app)
    if (date && typeof date === 'object') {
      ...
      // Handle timestamp with seconds property (serialized Timestamp)
      else if (date.seconds !== undefined) {
        const milliseconds = date.seconds * 1000;
        if (date.nanoseconds) {
          // Add nanoseconds if present (convert to milliseconds)
          // but we'll just use seconds for simplicity
        }
        dateObj = new Date(milliseconds);
      }
      // Handle object with _seconds property (another serialized format)
    ...
    if (!dateObj && typeof date === 'string') {
      ...
      // Skip empty strings
      if (trimmedDate === '' || trimmedDate === 'N/A') {
        return "N/A";
      }
      
      // Try to parse DD/MM/YYYY format (most common in your app)
    ...
    if (!dateObj && typeof date === 'number') {
      dateObj = new Date(date);
    }
    
    // If we still don't have a valid date object, return N/A
    if (!dateObj) {
      return "N/A";
    }
    ...
    // Format as DD/MM/YYYY using UTC methods to avoid timezone shifts
    ...
    // Silent fail - just return N/A without logging to console
    // This prevents the error messages you were seeing
    return "N/A";
  }
};

// Helper function to format bill number for display
const formatBillNumberDisplay = (billNumber) => {
...
// In lib/data.js - Update the createBoughtBill function

// In lib/data.js - Update the getBoughtBills function
  ...
  try {
    ...
    return snapshot.docs.map((doc) => {
      ...
      return {
        ...
        ...data,
        date: dateValue,
        // In data.js - Find the getBoughtBills function and update the expireDate formatting

// Inside the getBoughtBills function, around line 200-250
...
// Also update the createBoughtBill function to ensure expireDate is properly stored
export async function createBoughtBill(
  ...
  try {
    ...
    const itemsWithExpireDate = billItems.map((item) => {
      ...
      // Handle expire date - CRITICAL FIX
      if (item.expireDate) {
        console.log("Processing expireDate for item:", item.barcode, item.expireDate);
        ...
        console.log("Final timestamp for expireDate:", expireDateTimestamp);
      }
      ...
      return {
        ...
        expireDate: expireDateTimestamp, // Store as Firestore Timestamp
...
/**
 * Generate a sequential bill number starting with 26 followed by 4 digits
 * Format: 260001, 260002, 260003, etc.
 */
// lib/data.js - Update this function
export async function generateBillNumber() {
  try {
    ...
    const snapshot = await getDocs(billsRef);
    let maxBillNumber = 660000; // Changed from 260000 to 660000
 ...
 */
// lib/data.js - Update the createSoldBill function

// In lib/data.js - Updated createSoldBill function
// In lib/data.js - Replace the entire createSoldBill function
  ...
  try {
    ...
    if (returnData.barcode && returnData.returnQuantity) {
      ...
      if (!snapshot.empty) {
        const storeItem = snapshot.docs[0];
        const currentQuantity = storeItem.data().quantity;
        // Adjust store quantity based on the return quantity
...
const handleSubmitReturn = async () => {
  ...
  try {
    ...
    console.log("Submitting return with items:", JSON.stringify(preparedItems, null, 2));
    
    const result = await returnItemsToStore(selectedCompany.id, preparedItems);
...
export async function updateBoughtReturnItems(returnBillNumber, items) {
  try {
    if (!returnBillNumber) {
      throw new Error("Return bill number is required");
    }
    ...
    const createPromises = items.map(async (item) => {
      ...
      const returnRecord = {
        companyId: originalItems[0]?.companyId,
        returnBillNumber: returnBillNumber,
        barcode: item.barcode,
        name: item.name,
        billNumber: item.billNumber || "",
      ...
      const docRef = await addDoc(collection(db, "boughtReturns"), returnRecord);

      // Update store items with new quantities (deduct the new return quantity)
      const storeItemsRef = collection(db, STORE_ITEMS_COLLECTION);
      const storeQ = query(
        storeItemsRef,
        where("barcode", "==", item.barcode),
      ...
      if (!storeSnapshot.empty) {
        const storeItem = storeSnapshot.docs[0];
        const currentQuantity = storeItem.data().quantity;
        const newQuantity = currentQuantity - item.returnQuantity;
...
// Function to apply grayscale filter to an image file
export async function applyGrayscaleToImage(file) {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          reject(new Error('Failed to create grayscale image'));
        }
      }, 'image/jpeg', 0.85);
    };
...
// Function to store grayscale bill image
export async function storeGrayscaleBillImage(paymentId, imageFile) {
  try {
    if (!imageFile) {
    ...
    // Apply grayscale filter
    const grayscaleFile = await applyGrayscaleToImage(imageFile);
    
    // Upload to Firebase Storage
    const storage = getStorage();
    const storageRef = ref(storage, `boughtPayments/${paymentId}/${Date.now()}_grayscale.jpg`);
    await uploadBytes(storageRef, grayscaleFile);
    const downloadURL = await getDownloadURL(storageRef);
    
    return downloadURL;
  } catch (error) {
    console.error('Error storing grayscale bill image:', error);
    throw error;
  }
}

// Function to get base64 grayscale image
export async function getGrayscaleBase64Image(file) {
  return new Promise((resolve, reject) => {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    const img = new Image();
