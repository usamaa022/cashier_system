...
// Updated searchInitializedItems function with better error handling
export async function searchInitializedItems(searchQuery, searchType = 'barcode') {
...
// Fixed createBoughtBill function
export async function createBoughtBill(companyId, billItems, existingBillNumber = null) {
  try {
    console.log("Creating bought bill with companyId:", companyId, "type:", typeof companyId);
...
export async function searchBoughtBills(searchQuery) {
  try {
    ...
    if (searchQuery && searchQuery.length > 0) {
      ...
      // If no search query, get all bills
